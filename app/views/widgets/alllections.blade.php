			...
			<tr>
				...
				<td class="materialien">
				  ...
				  <ul class="dropdown-menu pull-right" role="menu">
					...
					<li><a href="{{ action('DownloadController@getFile', array('type' => 'pdf' , 'file' => str_replace(':','', $paper->papername ))) }}" onclick="javascript:_paq.push(['trackEvent', 'Text', 'Downloaded', '{{ $paper->papername }}'])"><small>{{ $paper->author }}</small><br> {{ $paper->papername }} <span class="glyphicon glyphicon-align-justify"></span></a></li>
					@endforeach

					@if( Video::available($video->videoname) || $role === 'Teacher' && $video->online)
					<li class="divider"></li>
					<li><a href="{{ action('LectionController@getNotesPDF', rawurlencode($video->videoname)) }}" onclick="javascript:_paq.push(['trackEvent', 'Notizen', 'Downloaded', '{{ $video->videoname }}'])">Notizen herunterladen <span class="glyphicon glyphicon-pencil"></span></a></li>				  
					@endif
					<li class="divider"></li>

					<li><a href="{{ action('DownloadController@getFile', array('type' => 'pdf' , 'file' => $video->section)) }}" onclick="javascript:_paq.push(['trackEvent', 'Weiterführende Literaturhinweise', 'Downloaded', '{{ $video->section }}'])"><small>Weiterführende Literaturhinweise <span class="glyphicon glyphicon-book"></span></small></a></li>
