...
$(document).ready ->
  ...
  # Aktuelle URL für AJAX Requests
  ...
  return
	...
	# In Mobilversionen nur auf dem horizontalen iPad AJAX erlauben. Wenn keine Verbindung besteht warnen.
...
#	if !(/Android|webOS|iPhone|iPad|iPod|BlackBerry|IEMobile|Opera Mini/i.test(navigator.userAgent))
	...
	$(document).on('click touchstart','.fp-cuepoint', ->
		...
		.fail ->
			...
			alert "Leider konnte keine Internetverbindung hergestellt werden. Überprüfen Sie bitte Ihre Verbindung und wenden Sie sich ggf. den technischen Support."
	)
	# AJAX POST --------------------------------------------------
	# CSRF Token abfragem.
	...
	$('#note-content').keyup ->
		...
		delay( ->
			...
			.done ->
				...
				ga('send','event','Notiz','ajax',(decodeURIComponent(currentURL.substr(50)) + ': Fähnchen ' + cuepointNumber.substr(23)))
			.fail ->
